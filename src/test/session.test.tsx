...
// type SessionResponse = { id: number; imgUrl: string; title: string; description: string; };
// export function SessionCard({ title }: Props) {
//   const [sessionName, setSessionName] = useState(title);
//   const [sessions, setSessions] = useState<SessionResponse[]>([]);

//   useEffect(() => {
//     fetch("escape-game/api/v1/sessions")
//       .then((res) => res.json())
...
//         {sessions?.map((slot) => (
//           <div key={slot.id} className="d-flex flex-row">
//             {slot.title}
//           </div>
//         ))}
//       </div>
